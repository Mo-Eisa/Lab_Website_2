...
function loadStatsPage(){
	...
	for(i = 2; i < table.rows.length; i++){
		cu = parseInt(table.rows[i].cells[2].innerHTML);
		vist = parseInt(table.rows[i].cells[3].innerHTML);
		if(cu > vist){
			table.rows[i].cells[4].innerHTML = "CU";
			wins++;
		}
		if(cu < vist){
			var temp = table.rows[i].cells[1].innerHTML;
			table.rows[i].cells[4].innerHTML = temp;
			losses++;
		}
	}
...
function loadPlayersPage(){
	...
	for(var player_i = 0; player_i < players.length; player_i++){
		text += '<a href="#" onclick = "switchPlayers(' + player_i + ')">' + players[player_i].name + '</a><br>';
	}
